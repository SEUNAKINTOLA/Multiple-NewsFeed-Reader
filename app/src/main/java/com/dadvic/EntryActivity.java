...
public class EntryActivity extends Activity {
	...
	*/
	...
	protected void onCreate(Bundle savedInstanceState) {
		...
		showRead = getIntent().getBooleanExtra(com.dadvic.EntriesListActivity.EXTRA_SHOWREAD, true);
	...
	protected void onResume() {
		...
		parentUri = FeedData.EntryColumns.PARENT_URI(uri.getPath());
		if (MainTabActivity.POSTGINGERBREAD) {
			com.dadvic.CompatibilityHelper.onResume(webView);
		}
		reload();
	}
				...
				if (canShowIcon) {
					...
					if (iconBytes != null && iconBytes.length > 0) {
						...
						if (bitmap != null) {
							...
							if (MainTabActivity.POSTGINGERBREAD) {
								com.dadvic.CompatibilityHelper.setActionBarDrawable(this, new BitmapDrawable(bitmap));
							} else {
	...
	private void setupButton(ImageButton button, final boolean successor, long date) {
		...
		if (!showRead) {
			queryString.append(Strings.DB_AND).append(com.dadvic.EntriesListAdapter.READDATEISNULL);
		}
	...
	private void nextEntry(boolean animate) {
		switchEntry(_nextId, animate, com.dadvic.Animations.SLIDE_IN_RIGHT, com.dadvic.Animations.SLIDE_OUT_LEFT);
	}
	
	private void previousEntry(boolean animate) {
		switchEntry(_previousId, animate, com.dadvic.Animations.SLIDE_IN_LEFT, com.dadvic.Animations.SLIDE_OUT_RIGHT);
	}
	...
	protected void onPause() {
		super.onPause();
		if (MainTabActivity.POSTGINGERBREAD) {
			com.dadvic.CompatibilityHelper.onPause(webView);
		}
