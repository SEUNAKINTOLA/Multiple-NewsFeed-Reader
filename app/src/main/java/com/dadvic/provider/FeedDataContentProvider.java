...
public class FeedDataContentProvider extends ContentProvider {
	...
	private static class DatabaseHelper extends SQLiteOpenHelper {
		...
		public void onCreate(SQLiteDatabase database) {
			...
			if (backupFile.exists()) {
				...
				com.dadvic.provider.OPML.importFromFile(backupFile, database);
			}
		}
		...
		public synchronized SQLiteDatabase getWritableDatabase() {
			...
			if (oldDatabaseFile.exists()) { // get rid of the old structure
				...
				try {
					...
					com.dadvic.provider.OPML.exportToFile(BACKUPOPML, newDatabase);
				} catch (Exception e) {
	...
	public int delete(Uri uri, String selection, String[] selectionArgs) {
		...
		if (table == TABLE_FEEDS) { // == is ok here
			com.dadvic.provider.OPML.exportToFile(BACKUPOPML, database);
		}
		if (count > 0) {
